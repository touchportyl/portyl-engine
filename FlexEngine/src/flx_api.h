...
// It is designed to optimize the passing of arguments in functions that use vector or SIMD (Single Instruction, Multiple Data) operations.
#define __FLX_CALL __vectorcall

// The compiler ensures that the starting address of the variable is divisible by X.
// Example usage: __FLX_ALIGN_AS(16) float myArray[4];
// Each float is 4 bytes, so the array is 16 bytes.
// Proper alignment can improve memory access performance, particularly when dealing with vectorized operations,
// SIMD (Single Instruction, Multiple Data) instructions, or GPU operations.
...
#define __FLX_ALIGN_AS(X) alignas(X)
