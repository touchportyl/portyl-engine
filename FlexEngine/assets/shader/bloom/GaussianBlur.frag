...
{             
    ...
    vec3 result = texture(screenTexture, tex_coord).rgb * weights[0];
    ...
    if(horizontal)
    {
        for(int i = 1; i < radius; i++)
        {
            ...
            result += texture(screenTexture, tex_coord + vec2(tex_offset.x * i, 0.0)).rgb * weights[i];
    ...
    else
    {
        for(int i = 1; i < radius; i++)
        {
            ...
            result += texture(screenTexture, tex_coord + vec2(0.0, tex_offset.y * i)).rgb * weights[i];
